...
/**
 * The ColorConverter class takes data from a external file and parses it into Color data  
 *
		...
		while(hexInput.compareTo("quit") != 0) {
			...
			if(c.getName() == null) {
				for(int i = 0; i < colorList.size(); i++) {
					if(c.getHexValue().equalsIgnoreCase(colorList.get(i).getHexValue())) {
						c.setName(colorList.get(i).getName());
					}
				}
			}
